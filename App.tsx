...
// Helper to generate consistent color for systems
const getFallbackSystemColor = (systemName: string) => {
  const colors = ['blue', 'green', 'purple', 'orange', 'teal', 'indigo', 'pink', 'cyan', 'rose', 'sky'];
  let hash = 0;
...
const App: React.FC = () => {
    ...
    try {
        ...
        if (faqError) {
            ...
            const mappedItems: FAQItem[] = (faqData || []).map((i: any) => ({
                ...
                category: i.category,
                type: i.type,
                needsUpdate: i.needs_update,
                isFavorite: i.is_favorite,
                isReusable: i.is_reusable,
                hasVideo: i.has_video,
                // CRITICAL: Ensure createdAt is number. Supabase might return bigint as string
        ...
        // Fetch Systems
        const { data: sysData, error: sysError } = await supabase.from('systems').select('*');
        if (sysError) {
             // Silent fallback for systems
             if (sysError.code !== 'PGRST205' && sysError.code !== '42P01') {
                 console.error("Error fetching Systems:", JSON.stringify(sysError, null, 2));
             }
             setSystems(DEFAULT_SYSTEMS); 
        } else if (sysData && sysData.length > 0) {
            setSystems(sysData.map((s: any) => s.name));
            const colors = sysData.reduce((acc: any, s: any) => ({ ...acc, [s.name]: s.color }), {});
            setSystemColors(colors);
        } else {
             setSystems(DEFAULT_SYSTEMS); // Fallback if table is empty
        }

        // Fetch Categories
        const { data: catData, error: catError } = await supabase.from('categories').select('*');
        if (catError) {
             if (catError.code !== 'PGRST205' && catError.code !== '42P01') {
        ...
        // Fetch Types
        const { data: typeData, error: typeError } = await supabase.from('types').select('*');
        if (typeError) {
             if (typeError.code !== 'PGRST205' && typeError.code !== '42P01') {
                console.error("Error fetching Types:", JSON.stringify(typeError, null, 2));
             }
        ...
        setToast({ message: `Erro de conexão: ${e.message || 'Desconhecido'}`, type: 'error' });
        setIsDemoMode(true);
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      const matchesSearch = 
        !searchLower ||
        (item.pfNumber && item.pfNumber.toLowerCase().includes(searchLower)) || 
  ...
  const getSystemColor = (systemName: string) => {
    ...
    return getFallbackSystemColor(systemName);
  };

  const handleSystemColorChange = async (system: string, colorKey: string) => {
    try {
        await supabase.from('systems').update({ color: colorKey }).eq('name', system);
        ...
        console.error(e);
        setToast({ message: 'Erro ao salvar cor', type: 'error' });
    }
  };
  ...
  const handleToggleFavorite = async (id: string, e: React.MouseEvent) => {
    ...
    try {
        await supabase.from('faq_items').update({ is_favorite: newVal }).eq('id', id);
    } catch (err) {
        // Revert
        setItems(prev => prev.map(i => i.id === id ? { ...i, isFavorite: !newVal } : i));
        setToast({ message: 'Erro ao atualizar favorito', type: 'error' });
    }
  };
  ...
  const executeDelete = async (id: string) => {
      try {
          ...
          console.error(err);
          // Fallback: reload data if delete failed
          fetchData();
  ...
  const handleBulkDelete = async () => {
    ...
    if (window.confirm(`Tem certeza que deseja excluir ${selectedIds.size} itens selecionados?`)) {
        try {
            ...
            const { error } = await supabase.from('faq_items').delete().in('id', idsToDelete);
            if (error) throw error;

            setItems(prevItems => prevItems.filter(i => !selectedIds.has(i.id)));
            setSelectedIds(new Set());
            setToast({ message: `${idsToDelete.length} itens excluídos`, type: 'success' });
        } catch (err) {
            console.error(err);
            setToast({ message: 'Erro na exclusão em massa', type: 'error' });
        }
    }
  };

  const handleMarkUpdated = async (item: FAQItem, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    const updatedHistory = [newHistory, ...(item.history || [])];

    // Optimistic
    setItems(prev => prev.map(i => {
      if (i.id === item.id) {
        return { ...i, needsUpdate: false, history: updatedHistory };
      }
      return i;
    }));
         ...
         const seedItems = [
             {
                 id: `seed-${now}-1`,
                 pf_number: '1234',
                 url: 'https://secullum.com.br/pf?id=1234',
                 question: 'Como configurar horário de verão no Ponto 4?',
                 content: 'Acesse Manutenção > Equipamentos > Cadastro > Configurações. Marque a opção Horário de Verão.',
                 summary: 'Acesse as configurações do equipamento no menu Manutenção e ative a flag de Horário de Verão.',
                 system: 'Ponto Secullum 4',
                 category: 'Suporte',
                 type: 'Configuração Equipamento',
                 needs_update: false,
                 is_favorite: true,
                 is_reusable: true,
                 has_video: false,
                 created_at: now,
                 history: [{ date: now, action: 'PF Criada (Seed)' }]
             },
             {
                 id: `seed-${now}-2`,
                 pf_number: '5678',
                 url: 'https://secullum.com.br/pf?id=5678',
                 question: 'Erro de Comunicação - Porta em uso',
                 content: 'Verifique se o serviço de comunicação não está travado ou se outro software está usando a porta.',
                 summary: 'O erro ocorre quando a porta TCP/IP está bloqueada. Reinicie o serviço de comunicação.',
                 system: 'Secullum Acesso',
                 category: 'Suporte',
                 type: 'Erro',
                 needs_update: true,
                 is_favorite: false,
                 ...
                 history: [{ date: now - 86400000, action: 'PF Criada (Seed)' }]
             },
             {
                 id: `seed-${now}-3`,
                 pf_number: '9012',
                 url: 'https://secullum.com.br/pf?id=9012',
                 question: 'Integração de Alunos - Academia',
                 content: 'Para integrar alunos, use a API v2. Documentação no link X.',
                 summary: 'Utilize a API v2 para integração de alunos no sistema Academia.',
                 system: 'Secullum Academia',
                 category: 'Comercial',
                 type: 'Integração',
                 needs_update: false,
                 is_favorite: false,
                 ...
                 history: [{ date: now - 172800000, action: 'PF Criada (Seed)' }]
             }
         ];
         
         const { error } = await supabase.from('faq_items').insert(seedItems);
         if (error) throw error;
         
         await fetchData();
         setToast({ message: 'Dados de teste gerados com sucesso!', type: 'success' });
     } catch (e) {
         console.error(e);
  ...
  const handleSave = async (isQuick = false) => {
    ...
    try {
      ...
      if (editingItem && !isQuick) {
        ...
        if (isQuick && !itemToSave.url && itemToSave.pfNumber) {
            itemToSave.url = `https://www.secullum.com.br/pt/perguntas-frequentes/${itemToSave.pfNumber}`;
        }
      }
      ...
      const dbPayload = {
          id: itemToSave.id,
          pf_number: itemToSave.pfNumber,
          url: itemToSave.url,
          question: itemToSave.question,
          content: itemToSave.content,
          summary: itemToSave.summary,
          notes: itemToSave.notes,
          system: itemToSave.system,
          category: itemToSave.category,
          type: itemToSave.type,
          needs_update: itemToSave.needsUpdate,
          is_favorite: itemToSave.isFavorite,
          is_reusable: itemToSave.isReusable,
          has_video: itemToSave.hasVideo,
          created_at: itemToSave.createdAt,
          history: itemToSave.history
      };

      const { error } = await supabase.from('faq_items').upsert(dbPayload);
      if (error) throw error;

      // Update Local State
      if (editingItem && !isQuick) {
          setItems(prev => prev.map(item => item.id === editingItem.id ? itemToSave : item));
      } else {
  ...
  const handleAddSystem = async () => {
    if (newSystemName && !systems.includes(newSystemName)) {
        ...
        try {
            await supabase.from('systems').insert({ name: newSystemName, color: randomColor });
            setSystems([...systems, newSystemName]);
            setSystemColors(prev => ({ ...prev, [newSystemName]: randomColor }));
            setNewSystemName('');
        } catch(e) {
            console.error(e);
            setToast({ message: 'Erro ao adicionar sistema', type: 'error' });
        }
    }
  };

  const handleRemoveSystem = async (name: string, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (confirm(`Remover sistema "${name}"?`)) {
        try {
            await supabase.from('systems').delete().eq('name', name);
            setSystems(systems.filter(s => s !== name));
        } catch(e) {
            console.error(e);
            setToast({ message: 'Erro ao remover sistema', type: 'error' });
        }
    }
  };
  ...
  const handleRemoveCategory = async (name: string, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleAddType = async () => {
    if (newTypeName && !types.includes(newTypeName)) {
        try {
            await supabase.from('types').insert({ name: newTypeName });
            setTypes([...types, newTypeName]);
            setNewTypeName('');
        } catch(e) {
             console.error(e);
             setToast({ message: 'Erro ao adicionar tipo', type: 'error' });
        }
    }
  };

  const handleRemoveType = async (name: string, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (confirm(`Remover tipo "${name}"?`)) {
        try {
            await supabase.from('types').delete().eq('name', name);
            setTypes(types.filter(t => t !== name));
        } catch(e) {
             console.error(e);
             setToast({ message: 'Erro ao remover tipo', type: 'error' });
        }
    }
  };
  ...
  const getHeaderAction = () => {
    if (editingItem) {
        return (
            <button 
                onClick={(e) => handleConfirmDelete(editingItem.id, e)} 
  ...
  if (isLoading) {
      return (
          <div className="min-h-screen bg-white dark:bg-slate-900 flex flex-col items-center justify-center text-gray-500 gap-4">
              <Loader2 size={40} className="animate-spin text-secullum-blue" />
              <p className="font-medium animate-pulse">Carregando base de dados...</p>
          </div>
      );
  }
    ...
    <div className={`min-h-screen flex flex-col md:flex-row font-sans transition-colors duration-300 ${darkMode ? 'bg-slate-900 text-slate-100' : 'bg-gray-100 text-slate-900'}`}>
      ...
      <aside className="w-full md:w-72 bg-secullum-dark dark:bg-slate-950 text-white flex-shrink-0 flex flex-col h-auto md:h-screen sticky top-0 overflow-y-auto z-20 shadow-2xl">
        ...
        <div className="px-6 py-2">
            {isDemoMode ? (
                <div className="flex items-center gap-2 px-3 py-2 bg-rose-900/40 rounded-lg border border-rose-800 text-rose-400 text-xs font-bold shadow-inner">
                    <WifiOff size={14} />
                    <span>Modo Demo (SQL Pendente)</span>
                </div>
            ) : (
          ...
          <div className="p-6 md:p-8 animate-in fade-in duration-500 max-w-[1600px] mx-auto min-h-screen flex flex-col">
            ...
            {filteredItems.length === 0 ? (
                 <div className="text-center py-24 bg-white dark:bg-slate-800 rounded-2xl border-2 border-dashed border-gray-200 dark:border-slate-700">
                    ...
                    <p className="text-sm text-gray-500 mt-2">Sua base de dados está vazia ou nenhum item corresponde aos filtros.</p>
                    
                    <div className="flex justify-center gap-3 mt-6">
                        <button onClick={clearFilters} className="text-secullum-blue font-bold flex items-center gap-2 bg-blue-50 dark:bg-blue-900/30 px-4 py-2 rounded-lg hover:bg-blue-100 transition-colors"><RotateCcw size={16}/> Limpar Filtros</button>
                        {!isDemoMode && items.length === 0 && (
                            <button onClick={handleSeedData} className="text-white font-bold flex items-center gap-2 bg-emerald-600 px-4 py-2 rounded-lg hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-900/20"><DatabaseZap size={16}/> Gerar Dados de Teste</button>
                        )}
                    </div>
                 </div>
            ) : (
                                ...
                                <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                                    {displayedItems.map(item => (
                                        <tr key={item.id} onClick={() => handleOpenModal(item)} className="cursor-pointer hover:bg-blue-50 dark:hover:bg-slate-700/50 group text-sm text-gray-700 dark:text-gray-300">
                                            ...
                                            <td className="p-4 text-right"><button onClick={(e) => handleConfirmDelete(item.id, e)} className="p-1.5 text-rose-500 hover:bg-rose-100 rounded opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={16}/></button></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                                ...
                                <div key={item.id} onClick={() => handleOpenModal(item)} className={`group relative flex flex-col h-full bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden cursor-pointer ${selectedIds.has(item.id) ? 'ring-2 ring-secullum-blue' : ''}`}>
                                    ...
                                    <div className="px-5 py-3 border-t border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex justify-between items-center text-xs">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            {/* Delete Button */}
                                            <button onClick={(e) => handleConfirmDelete(item.id, e)} className="p-1.5 text-gray-400 hover:text-rose-500 hover:bg-white dark:hover:bg-slate-700 rounded-md transition-all shadow-sm z-20 relative"><Trash2 size={14} /></button>
                                        </div>
                                    </div>
                                </div>
                            ))}
                                            ...
                                            <div key={item.id} onClick={() => handleOpenModal(item)} className={`bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border-l-4 cursor-pointer hover:shadow-lg hover:-translate-y-0.5 transition-all group relative ${item.needsUpdate ? 'border-l-rose-500' : 'border-l-secullum-green'}`}>
                                                <div className="flex justify-between items-start mb-2">
                                                    ...
                                                    <div className="flex items-center gap-1">
                                                        ...
                                                        <button onClick={(e) => handleConfirmDelete(item.id, e)} className="text-gray-300 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/30 p-1 rounded transition-colors" title="Excluir"><Trash2 size={14} /></button>
                                                    </div>
                                                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-full">
            <div className="lg:col-span-9 flex flex-col h-full gap-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1 min-h-[300px]">
                    ...
                    {/* MODIFIED AI SUMMARY AREA */}
                    <div className="flex flex-col h-full bg-indigo-50/30 dark:bg-indigo-900/10 rounded-xl border border-indigo-100 dark:border-indigo-500/20 shadow-sm overflow-hidden relative group">
                         <div className="bg-indigo-100/30 dark:bg-indigo-900/30 px-4 py-2 border-b border-indigo-100 dark:border-indigo-500/20 flex justify-between items-center">
                             ...
                             <button onClick={handleGenerateSummary} disabled={isGenerating || !formData.content} className="text-[10px] bg-white dark:bg-slate-800 hover:bg-blue-50 text-secullum-blue px-2 py-1 rounded shadow-sm transition-colors border border-blue-100 dark:border-slate-600 font-bold uppercase tracking-wide">{isGenerating ? 'Gerando...' : 'Gerar Agora'}</button>
                         </div>
                        <div className="relative flex-1 w-full">
                            <textarea value={formData.summary || ''} onChange={e => setFormData({...formData, summary: e.target.value})} className="w-full h-full p-4 bg-transparent outline-none resize-none text-sm leading-relaxed custom-scrollbar dark:text-white pb-12" placeholder="O resumo gerado aparecerá aqui..."></textarea>
                            {/* Gradient Overlay for Text Clipping Effect */}
                            {formData.summary && (
                                <div className="absolute bottom-0 left-0 right-0 h-12 bg-gradient-to-t from-indigo-50/50 via-indigo-50/10 to-transparent dark:from-slate-900/80 dark:to-transparent pointer-events-none"></div>
                            )}
                        </div>
